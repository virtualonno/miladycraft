EntityEvents.hurt('minecraft:player', event =>{
   if((event.entity.hasEffect('kubejs:sanctuary')) 
      && (!event.entity.hasEffect('kubejs:flagged')) 
      && (!event.entity.hasEffect('kubejs:deserter'))
      && (!event.entity.hasEffect('kubejs:fated_death'))
   ){
      event.cancel();
   }
})
